...
class FlutterAlertConfig {
  ...
  func getAlertConfig() -> AlertConfiguration {
    return AlertConfiguration(
      ...
      sound: (_sound == nil) ? .default : AlertConfiguration.AlertSound.named(_sound!))
  }
}
...
public class LiveActivitiesPlugin: NSObject, FlutterPlugin, FlutterStreamHandler {
  ...
  public static func register(with registrar: FlutterPluginRegistrar) {
    ...
    let activityStatusChannel = FlutterEventChannel(
      name: "live_activities/activity_status", binaryMessenger: registrar.messenger())
    let pushToStartTokenUpdatesChannel = FlutterEventChannel(
      name: "live_activities/push_to_start_token_updates", binaryMessenger: registrar.messenger())
  ...
  private func initializationGuard(result: @escaping FlutterResult) {
    if self.appGroupId == nil || self.sharedDefault == nil {
      result(
        FlutterError(
          code: "NEED_INIT",
          message: "you need to run 'init()' first with app group id to create live activity",
          details: nil))
    }
  }
    ...
    if #available(iOS 16.1, *) {
      ...
      case "init":
        ...
        if let appGroupId = args["appGroupId"] as? String {
          ...
          result(
            FlutterError(
              code: "WRONG_ARGS", message: "argument are not valid, check if 'appGroupId' is valid",
              details: nil))
        }
      ...
      case "createActivity":
        ...
        guard let args = call.arguments as? [String: Any] else {
          result(
            FlutterError(code: "WRONG_ARGS", message: "Unknown data type in argument", details: nil)
          )
          return
        }
        ...
        if let data = args["data"] as? [String: Any] {
          ...
          createActivity(
            data: data, removeWhenAppIsKilled: removeWhenAppIsKilled, staleIn: staleIn,
            result: result)
        } else {
          result(
            FlutterError(
              code: "WRONG_ARGS", message: "argument are not valid, check if 'data' is valid",
              details: nil))
        }
        break
      case "updateActivity":
        ...
        if let activityId = args["activityId"] as? String, let data = args["data"] as? [String: Any]
        {
          ...
          let alertConfig =
            (alertTitle == nil || alertBody == nil)
            ? nil : FlutterAlertConfig(title: alertTitle!, body: alertBody!, sound: alertSound)

          updateActivity(
            activityId: activityId, data: data, alertConfig: alertConfig, result: result)
        } else {
          result(
            FlutterError(
              code: "WRONG_ARGS",
              message: "argument are not valid, check if 'activityId', 'data' are valid",
              details: nil))
        }
        break
      case "endActivity":
        ...
        if let activityId = args["activityId"] as? String {
          ...
          result(
            FlutterError(
              code: "WRONG_ARGS", message: "argument are not valid, check if 'activityId' is valid",
              details: nil))
        }
        break
      ...
      case "createOrUpdateActivity":
        ...
        if let data = args["data"] as? [String: Any], let customId = args["customId"] as? String {
          ...
          createOrUpdateActivity(
            data: data, customId: customId, removeWhenAppIsKilled: removeWhenAppIsKilled,
            staleIn: staleIn, result: result)
        } else {
          result(
            FlutterError(
              code: "WRONG_ARGS",
              message: "argument are not valid, check if 'data', 'customId' is valid", details: nil)
          )
        }
        break
      default:
      ...
      result(
        FlutterError(
          code: "WRONG_IOS_VERSION", message: "this version of iOS is not supported", details: nil))
    }
  }

  @available(iOS 16.1, *)
  func createActivity(
    data: [String: Any], removeWhenAppIsKilled: Bool, staleIn: Int?, customId: String? = nil,
    ...
    center.requestAuthorization(options: [.alert, .sound, .badge]) { granted, error in
      if let error = error {
        result(
          FlutterError(
            code: "AUTHORIZATION_ERROR", message: "authorization error",
    ...
    let initialContentState = LiveActivitiesAppAttributes.LiveDeliveryData(
      ...
      routeTextColor: data["routeTextColor"] as? String ?? "",
      headsign: data["headsign"] as? String ?? "",
    ...
    if #available(iOS 16.2, *) {
      let activityContent = ActivityContent(
        ...
        staleDate: staleIn != nil
          ? Calendar.current.date(byAdding: .minute, value: staleIn!, to: Date.now) : nil)
      do {
        ...
        result(
          FlutterError(
            code: "LIVE_ACTIVITY_ERROR", message: "can't launch live activity",
            details: error.localizedDescription))
      }
    } else {
      do {
  ...
  @available(iOS 16.1, *)
  func updateActivity(
    activityId: String, data: [String: Any?], alertConfig: FlutterAlertConfig?,
    result: @escaping FlutterResult
  ) {
    Task {
  ...
  @available(iOS 16.1, *)
  func createOrUpdateActivity(
    data: [String: Any], customId: String, removeWhenAppIsKilled: Bool, staleIn: Int?,
    result: @escaping FlutterResult
  ) {
    Task {
      guard let customUUID = UUID(uuidString: customId) else {
        result(
          FlutterError(
            code: "WRONG_ARGS", message: "argument are not valid, check if 'customId' is valid",
            details: nil))
        return
      }
      ...
      let existingActivity = activities.first {
        $0.attributes.id == customUUID && $0.activityState != .dismissed
      ...
      if let activityId = existingActivity?.id {
        ...
        createActivity(
          data: data, removeWhenAppIsKilled: removeWhenAppIsKilled, staleIn: staleIn,
          customId: customId, result: result)
      }
    }
  }
  ...
  func getActivityState(activityId: String, result: @escaping FlutterResult) {
    Task {
      if let matchingActivity = Activity<LiveActivitiesAppAttributes>.activities.first(where: {
        $0.id == activityId
      }) {
  ...
  public func application(
    ...
    var queryResult: [String: Any] = Dictionary()

    queryResult["queryItems"] = components?.queryItems?.map({ (item) -> [String: String] in
      var queryItemResult: [String: String] = Dictionary()
